...
app.post('/api/analyze-image', upload.single('file'), async (req, res) => {
    ...
    if (req.file) {
        const analysis = await analyzeImage(`uploads/${req.file.originalname}`);
        console.log(analysis.length);
        await res.status(200).send(analysis);
    }
    else {
        res.send("no file")
    }
        ...
        messages: [
            {
                ...
                content: [
                    ...
                    {
                        type: "image_url",
                        image_url: `data:image/jpeg;base64, ${convertToBase64(filePath)}`
                    },
                ],
            },
        ],
